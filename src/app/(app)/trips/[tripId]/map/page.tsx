...
import { Map, MapPin, CalendarDays, Clock } from 'lucide-react';
...
export default function MapPage() {
  ...
  const { events, loading } = useEvents(tripId);
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            {eventsWithCoords.length === 0
              ? 'Agrega coordenadas a tus eventos para verlos aqui'
              : `${eventsWithCoords.length} ubicacion${eventsWithCoords.length !== 1 ? 'es' : ''} en el mapa`}
          </p>
        </div>
      </div>
